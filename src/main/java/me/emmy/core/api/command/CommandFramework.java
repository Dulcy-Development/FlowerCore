...
public class CommandFramework implements CommandExecutor {
    ...
    private void defaultCommand(CommandArgs args) {
        ...
        if (args.getSender().hasPermission(FlowerCore.getInstance().getConfigHandler().getConfig("messages.yml").getString("anti-syntax-bypass-perm"))) {
            if (parts.length > 1) {
            ...
            args.getSender().sendMessage(CC.translate(FlowerCore.getInstance().getConfigHandler().getConfig("messages.yml").getString("anti-syntax").replace("%argument%", args.getLabel())));
        }
    }
