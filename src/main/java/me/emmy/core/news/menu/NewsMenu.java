...
public class NewsMenu extends Menu {
	...
	public NewsMenu() {
		this.refillGlassButton = new RefillGlassButton(
				...
				FlowerCore.getInstance().getConfigHandler().getConfig("menus/news.yml").getInt("refill-glass.data", 15)
		);
	}

	@Override
	public String getTitle(Player player) {
		return CC.translate(FlowerCore.getInstance().getConfigHandler().getConfig("menus/news.yml").getString("title"));
	}
	...
	public Map<Integer, Button> getButtons(Player player) {
		...
		ConfigurationSection serversSection = FlowerCore.getInstance().getConfigHandler().getConfig("menus/news.yml").getConfigurationSection("items");
		...
		ConfigurationSection refillGlassSection = FlowerCore.getInstance().getConfigHandler().getConfig("menus/news.yml").getConfigurationSection("refill-glass");
	...
	@Override
	public int getSize() {
		FileConfiguration newsConfig = FlowerCore.getInstance().getConfigHandler().getConfig("menus/news.yml");
		if (newsConfig != null) {
