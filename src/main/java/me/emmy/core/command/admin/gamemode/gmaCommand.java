    ...
    public void onCommand(CommandArgs args) {
        if (args.length() > 0) {
            ...
            if (player.getGameMode().equals(GameMode.ADVENTURE)) {
                player.sendMessage(CC.translate(FlowerCore.getInstance().getConfigHandler().getConfig("messages.yml").getString("gamemode.adventure.is_already")));
            } else {
                player.sendMessage(CC.translate(FlowerCore.getInstance().getConfigHandler().getConfig("messages.yml").getString("gamemode.adventure.switched")));
