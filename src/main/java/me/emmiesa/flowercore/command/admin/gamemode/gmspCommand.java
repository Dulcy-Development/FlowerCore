    ...
    public void onCommand(CommandArgs args) {
        if (args.length() > 0) {
            ...
            if (target.getGameMode().equals(GameMode.SPECTATOR)) {
                ...
                args.getPlayer().sendMessage(CC.translate("&bYou've set &r" + FlowerCore.getInstance().getProfileManager().getRank(targetUUID).getPrefix() + target.getName() + "&b's gamemode to &3Spectator&b."));
                target.setGameMode(GameMode.SPECTATOR);
                target.sendMessage(CC.translate("&bYour gamemode has been set to &3spectator &bby &r" + FlowerCore.getInstance().getProfileManager().getRank(playerUUID).getPrefix() + args.getPlayer().getName() + " &b."));
