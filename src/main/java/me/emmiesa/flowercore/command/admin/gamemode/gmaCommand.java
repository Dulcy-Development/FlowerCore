    ...
    public void onCommand(CommandArgs args) {
        if (args.length() > 0) {
            ...
            if (target.getGameMode().equals(GameMode.ADVENTURE)) {
                ...
                args.getPlayer().sendMessage(CC.translate("&bYou've set &r" + FlowerCore.getInstance().getProfileManager().getRank(targetUUID).getPrefix() + target.getName() + "&b's gamemode to &3Adventure&b."));
                target.setGameMode(GameMode.ADVENTURE);
                target.sendMessage(CC.translate("&bYour gamemode has been set to &3adventure &bby &r" + FlowerCore.getInstance().getProfileManager().getRank(playerUUID).getPrefix() + args.getPlayer().getName() + " &b."));
